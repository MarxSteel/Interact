 ...
 <div class="modal-dialog">
  <div class="modal-content">
   <div class="modal-header bg-purple-400">
	<button type="button" class="close" data-dismiss="modal">&times;</button>
	 <h6 class="modal-title">Atualizar E-mail</h6>
   </div>
   <div class="modal-body">
	<h3>Atualizar E-Mail</h3>
	<form name="TrocaEmail" id="TrocaEmail" method="post" action="" enctype="multipart/form-data">
	<div class="form-group">
   ...
   <div class="modal-footer"><br /><br /><br />
    <button type="button" class="btn btn-link" data-dismiss="modal">Fechar	</button>
    <input name="TrocaEmail" type="submit" class="btn bg-purple-400" value="Atualizar Cadastro"  />
    </form>
    <?php 
    if(@$_POST["TrocaEmail"])
    {
     $NovoEmail = $_POST['mail'];
      $AtualizaMail = $db->query("UPDATE ic_clube SET mailContato='$NovoEmail' WHERE id='$ClID'");
       if ($AtualizaMail) {
       	$DataLog = date('Y-m-d H:i:s');
       	$Descricao = "Atualizado E-Mail<br />Clube: " . $ClNome . "<br />E-mail antigo: " . $MailClube . "<br />Novo E-Mail: " . $NovoEmail;
       	$InsereLog2 = $db->query("INSERT INTO logs (user, logCod, descreve, dtCadastro) VALUES ('$nickname', '206', '$Descricao', '$DataLog')");
       	 if ($InsereLog2) {
		  echo "<script>location.href='vClube.php?ID=" . $ClID . "&sucesso=bg-success&evento=Atualizar%20E-Mail&mensagem=E-mail%20Atualizado%20com%20Sucesso!'</script>";
       	 }
       	 else{
		echo "<script>location.href='vClube.php?ID=" . $ClID . "&sucesso=bg-danger&evento=Atualizar%20E-Mail&mensagem=Não%20Foi%20possível%20Atualizar%20o%20E-Mail.%20Erro:%200x27'</script>";
       	 }
       	}
       else{
		echo "<script>location.href='vClube.php?ID=" . $ClID . "&sucesso=bg-danger&evento=Atualizar%20E-Mail&mensagem=Não%20Foi%20possível%20Atualizar%20o%20E-Mail.%20Erro:%200x28'</script>";
       }
    }
    ?>
   </div>
 ...
 <div class="modal-dialog">
  <div class="modal-content">
   <div class="modal-header bg-orange-400">
	<button type="button" class="close" data-dismiss="modal">&times;</button>
	 <h6 class="modal-title">Atualizar Rotary Club</h6>
   </div>
   <div class="modal-body">
   ...
   <div class="modal-footer"><br /><br /><br />
    ...
    if(@$_POST["TrocaRC"])
    {
     $NovoRotary = $_POST['rcp'];
      $AtualizaRC = $db->query("UPDATE ic_clube SET rcPadrinho='$NovoRotary' WHERE id='$ClID'");
       if ($AtualizaRC) {
       	$DataLog = date('Y-m-d H:i:s');
       	$Descricao = "Atualizado Rotary Club Patrocinador<br />Clube: " . $ClNome . "<br />Rotary Club antigo: " . $Rotary . "<br />Novo Club Patrocinador: " . $NovoRotary;
